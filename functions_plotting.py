...
def plot_step_in_env(ax, info):
    ...
    for agent in agents:
        ...
        alpha_list.append(0.2 if agent.curr_node == agent.get_goal_node() else 1)
    ...
    if 'to_title' in info:
        title_str += f"{info['to_title']}\n "
    if 'img_dir' in info:
        title_str += f"Map: {info['img_dir'][:-4]}\n "
    if 'i' in info:
        title_str += f"(iteration: {info['i'] + 1})\n"
